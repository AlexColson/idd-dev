...
- **Property-based testing (PBT) — preferred when applicable.** Use `proptest` for functions with well-defined invariants: parsers, serializers, validators, state machines, ID generators, roundtrip conversions, file operations, search correctness. PBT should be the default choice over hand-written examples when the function has input/output invariants that can be expressed as properties. **PBT must cover both positive and negative cases:** valid inputs should roundtrip and preserve invariants; invalid inputs (malformed data, unknown enum variants, missing required fields, boundary violations) must be rejected. Hand-written examples complement PBT for edge cases and regression tests.
- **File operations invariants** — test atomic writes (temp+rename), read-after-write consistency, directory creation, search results are subset of registry.
